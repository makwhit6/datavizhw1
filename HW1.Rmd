...
course_click <- MJ2 %>% 
  ...
  summarize(total_sum = sum(sum_click)) %>% 
...
ggplot(subset(MJ2, activity_type %in% c("oucontent",
  ...
  facet_grid(cols = vars(activity_type)) +
  coord_flip() +
  theme_minimal() +
  labs(title = "Student Interaction with Course Content",
       caption = "Plot shows top 5 most interacted with content areas.",
       y = "Number of student clicks per course",
       x = "")
```
...
stu_assess_avg <- stu_assess %>%
  mutate(score_avg = mean(score) + 3*sd(score)) %>%
  group_by(id_student, date_submitted) %>%
  summarize(stu_assess_avg = mean(score),
